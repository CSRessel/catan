...
public class Game {
	 ...
	 * @return whether p was able buy the thing
	 */
	private boolean buyObject(Player p, int choice) {
		...
		if (choice == 1) {
			do {
				...
				boolean goodSpot = board.placeRoad(loc, p);
			} while (!goodSpot);
		}
		else if (choice == 2) {
			do {
				...
				boolean goodSpot = board.placeStructure(loc, p);
			} while (!goodSpot);
		}
		else if (choice == 3) {
			VertexLocation loc = new VertexLocation(xCoord, yCoord, orient);
